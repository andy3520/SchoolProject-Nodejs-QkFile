function changeToGreen(e) {
  $(".title-upload").addClass("text-color-active"), $(".file-upload").addClass("active"), $(".upload-btn").addClass("active"), $(".transform-color").addClass("active"), $("#findcode").addClass("active"), $("#findpass").addClass("active"), $("#noFile").text(e.replace("C:\\fakepath\\", ""))
}

function changeToYellow() {
  $(".title-upload").removeClass("text-color-active"), $(".file-upload").removeClass("active"), $(".upload-btn").removeClass("active"), $("#findcode").removeClass("active"), $("#findpass").removeClass("active"), $(".transform-color").removeClass("active"), $("#noFile").text("No file chosen...")
}
...
function sendAjaxUpload() {
  if (!passFile) {
    $("#errorModal").modal("show"), $("#errorMessage").html("411 - File lớn hơn mức quy định là 50 MB");
  } else {
...
uploadField.onchange = function(){
  ...
  /^\s*$/.test(e) ? changeToYellow() : changeToGreen(e);
  if(this.files[0].size > 5e7){
    this.value = "";
    passFile = false;
  } else passFile = true;
  ...
  if (0 === o.val().length) $("#errorModal").modal("show"), $("#errorMessage").html("Vui lòng nhập mã tìm kiếm");
    ...
    e.code = o.val(), e.pass = a.val(), "" !== e.pass && void 0 !== e.pass || (e.pass = " "), $.ajax({
      ...
      success: e => {
        ...
        let o = e.data,
          a = Number(o.fileSize) / 1024,
          s = Math.round(a) < 1e3 ? Math.round(a) + "KB" : Math.round(a / 1024) + "MB";
